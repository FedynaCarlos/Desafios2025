# 🚀 Desafíos y repasos técnicos para crecer como Desarrollador Web Full Stack

Hola, soy **Juan Carlos Fedyna** y este repositorio contiene una serie de desafíos y ejercicios prácticos que estoy realizando para demostrar y consolidar mi progreso como **desarrollador web full stack**.

Cada día aborda un aspecto clave del desarrollo web, combinando lógica, diseño modular, interacción con el usuario y buenas prácticas. Algunos desafíos cumplen un doble objetivo: ejercitar habilidades nuevas y reforzar fundamentos esenciales.

---

## 🌟 Objetivo del proyecto

- Consolidar mis conocimientos con desafíos reales y progresivos.  
- Mostrar públicamente mi evolución como desarrollador.  
- Construir un portfolio funcional, escalable y profesional.  
- Publicar cada avance en GitHub y LinkedIn.

---

## 📚 Desafíos realizados

| 🔵 Desafío | 🧠 Tema                                         | 🛠️ Tecnologías principales                | 🔵 Estado       |
|-----------:|------------------------------------------------|-------------------------------------------|-----------------|
| Día 1      | Variables y tipos de datos                      | JavaScript ES6+, Git                      | ✅ Completado    |
| Día 2      | Lógica + diseño con Flexbox                     | JavaScript, Flexbox, estructura semántica | ✅ Completado    |
| Día 3      | Lista de tareas en React (repaso + práctica)   | React.js, JSX, useState, CSS Modules     | ✅ Completado    |
| Día 4      | Lista de tareas con React & Vite (estado, eventos, estilos) | React.js, Vite, useState, CSS             | ✅ Completado    |
| Día 5      | ...                                            |                                           | ⏳ Pendiente     |
| Día 6      | ...                                            |                                           | ⏳ Pendiente     |
| Día 7      | ...                                            |                                           | ⏳ Pendiente     |
| Día 8      | ...                                            |                                           | ⏳ Pendiente     |
| Día 9      | Proyecto final integrador                       | Git, lógica + estructura modular          | ⏳ Pendiente     |

---

## 🧱 Día 4 - Lista de tareas con React & Vite

Este proyecto consiste en una aplicación sencilla para administrar tareas, con una interfaz centrada tipo **card** y un fondo fijo con imagen para mejor experiencia visual.

🔹 **Objetivo pedagógico:**  
Practicar React con hooks (`useState`), manejo de eventos, condicionales, y estilos CSS modernos. Además, usar Vite para un entorno de desarrollo rápido y optimizado.

🔹 **Resultado final:**  
Una app funcional donde se puede:

- Agregar nuevas tareas (no vacías).  
- Marcar o desmarcar tareas como **completadas** (checkbox).  
- Eliminar tareas individualmente.  
- Visualizar la lista en una tarjeta centrada con diseño responsive.  
- Fondo fijo con imagen para un look profesional.

---

## ⚙️ Cómo ejecutarlo localmente

1. Asegurate de tener **Node.js** y **npm** instalados.  
2. Cloná el repositorio:  
   ```bash
   git clone https://github.com/tu-usuario/nombre-repo.git
   cd nombre-repo/dia4-react-tareas
3. Instalá las dependencias:
 npm install
4. Ejecutá el servidor de desarrollo:
   npm run dev
5. Abrí la URL que te indica la consola (usualmente http://localhost:5173) en tu navegador.

📁 Estructura principal del proyecto

dia4-react-tareas/
├── public/                 # Archivos estáticos (ej: imagen fondo)
├── src/
│   ├── assets/             # Recursos como imágenes (ej: react.svg)
│   ├── App.jsx             # Componente principal con la lógica y UI
│   └── main.jsx            # Entrada principal para React
├── index.html              # Archivo HTML base
├── package.json            # Dependencias y scripts npm
├── vite.config.js          # Configuración de Vite
└── README.md               # Este archivo

